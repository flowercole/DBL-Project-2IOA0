...
loadForceGraph = () => {
  ...
  // load the json file and save it in global variable
  //d3.json("miserables.json", function(data) {
    //  graph_data = data;
      // Load a copy of graph data into a variable og_data that we can access when resetting
      // Don't know why it's done like this but it was the only way I could get it to work
  ...
  //put vertices in array nodes, and edges in edges
  ...
  //create the edges based on the csv
  console.log(csv)
  for (var i = 0; i < 100; i++) {
  ...
  function renderDisplay(vertices, edges) {
      ...
      d3.selectAll("circle")
  		.attr("fill", function(d) {if (!graph_data.selected_nodes.includes(d)) {return colors} else {return inverseColors} }) //sets the color of circle          //!!!!!!!
  ...
  //update for settings               !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! and everything underneath
