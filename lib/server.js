...
server.unifiedServer = function(req, res){
    ...
    if(method == 'post' && trimmedPath == 'upload'){
        ...
        form.on('fileBegin', function(name, file){
            ...
            file.path = `${form.uploadDir}/${Path.basename(file.name)}_${Math.floor(Date.now() / 1000)}${Path.extname(file.name)}`;
        });
        ...
        form.parse(req, function(err, fields, files) {
            if(!err){
                ...
                res.write(JSON.stringify({'type':'success','message' : 'File uploaded successfully.'}));
                res.end();
            }else {
