...
server.unifiedServer = function(req, res){
    ...
    if(method == 'post' && trimmedPath == 'upload'){
        ...
        form.on('fileBegin', function(name, file){
            ...
            var file_name = `${Path.basename(file.name)}_${Math.floor(Date.now() / 1000)}${Path.extname(file.name)}`;
        ...
        form.parse(req, function(err, fields, files) {
            if(!err){
                ...
                res.write(JSON.stringify({'type':'success','message' : 'File uploaded successfully.', 'file_name' : new_file_name}));
                res.end();
            } else {
