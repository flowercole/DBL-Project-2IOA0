...
server.unifiedServer = function(req, res){
    ...
    if(method == 'post' && trimmedPath == 'upload'){
        const allowedMIMETypes = ['application/vnd.ms-excel', 'text/plain']
        ...
        form.on('fileBegin', function(name, file){
            ...
            var new_name = `${Path.basename(file.name, '.csv')}_${Math.floor(Date.now() / 1000)}${Path.extname(file.name)}`;
...
server.router = {
    ...
    'visualization': handlers.visualization,
    'main': handlers.main,
    'comp': handlers.comp
};
