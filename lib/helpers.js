...
helpers.getComponents = (compName, callback) => {
    // Fetch view  based on request
    typeof(compName) === "string" && compName.length > 0 ? compName : false;
    if(compName){
        const compDir = path.join(__dirname,'/../views/components/');
        fs.readFile(compDir+compName+'.html', 'utf8', function(err,str){
            if(!err && str && str.length > 0){
                callback(false, str);
            }else {
                callback(' not found');
